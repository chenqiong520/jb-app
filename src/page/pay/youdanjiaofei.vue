<template>
  <div class="ydjf">
    <x-header  v-show="!showOrder" :left-options="{backText: ''}">有单缴费查询</x-header>
      ...
      <div>
        <div  v-show="!showOrder">
          <group class="no_margin">
            ...
            <x-input title="业务单据号" v-model="orderNo"></x-input>
          </group>
          <div class="notice">
            <div>温馨提示：受人民银行清算系统限制每周五至周六及每日19:00-次日7:00为非交易时段，不能缴费。</div>
          </div>
          <div class="btn_group">
            <x-button class="submit_btn next_btn query_btn" :class="idCard ==='' || orderNo=== ''? 'disabled': ''" :disabled="idCard ==='' || orderNo=== ''"  @click.native="queryPayOrder">查询缴费</x-button>
          </div>
        </div>
        <div v-show="showOrder">
...
export default {
  ...
  methods: {
    ...
    queryPayOrder () {
      const params = api.getParam('lhjy06', {aac002: this.idCard.trim(), aae097: this.orderNo.trim()})
